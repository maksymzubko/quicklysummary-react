...
class AuthApi {
    ...
    async login(data: Login): Promise<LoginResponse> {
        ...
        if (response.status === 200 || response.status === 201) {
            ...
            localStorage.setItem('quickly_summary_token', JSON.stringify({id, email, access_token, uuid, isCustom: false, name, avatar}))
    ...
    async loginViaToken(token: string): Promise<LoginResponse> {
        ...
        if (response.status === 200 || response.status === 201) {
            const {id, email, access_token, uuid, name, avatar } = response.data;
            localStorage.setItem('quickly_summary_token', JSON.stringify({id, email, access_token, uuid, isCustom: true, name, avatar}))
            return response.data;
        }
